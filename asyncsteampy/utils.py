...
def parse_price(price: str) -> decimal.Decimal:
    tokens = re.search(r"\D?(\d*)(\.|,)?(\d*)", price, re.UNICODE)
...
def get_key_value_from_url(url: str, key: str, case_sensitive: bool = True) -> str:
    ...
    if case_sensitive:
        ...
        return CIMultiDict(urlparse.parse_qs(params))[key][0]
...
def normalize_params(params: dict) -> dict:
    ...
    normalized_params.update(
        {key: str(value).lower() for key, value in normalized_params.items() if type(value) is bool}
    )
